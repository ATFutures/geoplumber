...
installation, especially if you are already an R user, with the
following command:
...
#>   --quiet CMD INSTALL  \
#>   '/tmp/RtmpTyahxL/devtools602734de7dfd/ATFutures-geoplumber-f45cceebe52e77616e579635c97250f36aa2394a'  \
...
running) to keep you up to date with
updates.
...
Serve the `geoplumber::meta` dataset (data.frame) at a “/api/data”
...
``` r
#' Serve geoplumber::meta from /api/data
#' @get /api/data
get_meta <- function(res) {
  geojson <- geojsonio::geojson_json(geoplumber::meta)
  res$body <- geojson
  res
}
```
...
<img src="man/figures/gp.png" title="CDRC London traffic data on geoplumber" alt="CDRC London traffic data on geoplumber" width="70%" style="display: block; margin: auto;" />
