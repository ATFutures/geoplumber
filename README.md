...
geoplumber is R package that extends [`plumber`](https://github.com/trestletech/plumber), which was designed for creating web APIs with R. It supports [React](https://reactjs.org/) frontends at present (it may support other frontend frameworks such as VueJS in the future) and geographic data, building on [`sf`](https://github.com/r-spatial/sf).
...
Currently geoplumber uses Facebook's `create-react-app` (CRA) npm package to deal with underlying app management (including building and running) to keep you up to date with updates.
...
Trying to test drive the package but it is not always the case.
...
library(geoplumber)
gp_create("my_app")
#> Initializing project at: my_app
#> Ready to build app in directory: my_app
